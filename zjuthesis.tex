...
\documentclass[
    ...
    % Thesis Info
    ClassNumber     = TP181,          % 按中国图书分类法，学位办网上可查
    Security        = 无,              % 注明密级与保密期限，仅限于论文课题来源于国防军工项目，如没有请填：无
    StudentName     = 江布力·加尔肯别克,
    StudentID       = 22360097,
    AdvisorName     = 熊蓉,
    Grade           = 2023,            % only the year, no '级'
    Major           = 控制工程,
    Department      = 工程师学院,
    SubmitDate      = 2026年3月20日,
    MajorFormat     = general,
    ...
    % Title
    Title           = 基于高斯过程回归的模型预测控制器,
    TitleEng        = {{Gaussian Process Regression based Model Predictive Controller}}
]{zjuthesis}

%% Uncomment the following lines if you need multi line titles on cover pages
\titletwolines{基于高斯过程回归}{的模型预测控制器}
% \titleengtwolines{Gaussian Process Regression}{Model Predictive Controller}
% \titlethreelines{毕业论文题目第一行}{毕业论文题目第二行}{毕业论文题目第三行}
\titleengthreelines{Gaussian Process Regression}{based Model Predictive}{Controller}
...
\raggedbottom % To avoid the adjust of the space if the next section has no space in this page
